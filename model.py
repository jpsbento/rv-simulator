...
days = 3 						#How long in days you will be observing straight
obsPeriod = 7 					#Hours of observing at night
spaceLength = [1,1,1,1] 		#List of lengths of spaces between observation periods in hours.
medlsVal = [] 					#Median amplitude value of the smoothed lomb scargle. This is determined in the program
iterations = 1000	#Number of times you would like to run the script


M_bol_sun=4.7554
d=2750 #distance to M67 in parsecs

masses_out=[]
detection_levels=[]
detection_levels_sigma=[]
lumins=[]
M_bol=[]
m=[]

masses=numpy.arange(1.360,1.389,0.001)
#masses=[1.4]
...
#			pyplot.show()
			...
			smoothls = movingaverage(ls.power, 10) #smooths lomb-scargle
			medls = numpy.median(smoothls)         #Calculates the median of the smoothed lomb-scargle
			medlsVal = ['Median Power Smooth LS: ', medls]
			...
			if doPlot:
				...
				pyplot.plot(ls.freq*1e6, smoothls) #edited by carlos
				pyplot.plot(ls.freq*1e6, ls.power)
				pyplot.axhline(medls)
				pyplot.axhline(medls*2)
	...
	#			pyplot.plot(ls.freq, numpy.ones(len(smoothls))*medls*2, ':')	
			...
			for k in numpy.arange(len(smoothls)):
				if smoothls[k] == max(smoothls):
					smoothInd = k
			detection_level=max(smoothls)/medls
#			print (ls.freq[freqInd])*1000000, (ls.freq[smoothInd])*1000000, max(ls.power), max(smoothls), detection_level, goodStar
			...
			temp_detection_level.append(detection_level)
#		if doPlot: pyplot.show()
		output.writerow(medlsVal)
#		print medlsVal
#		print ' '
		masses_out.append(mass)
		detection_levels.append(numpy.mean(temp_detection_level))
		detection_levels_sigma.append(numpy.std(temp_detection_level))
		lumins.append(lumin)
		M_bol.append(-2.5*numpy.log10(lumin)+M_bol_sun)
		raw_m = -2.5*numpy.log10(lumin)+M_bol_sun-5*(1-numpy.log10(d))
		new_m = ("%0.1f" % raw_m)
		m.append(new_m)
#print masses_out
#print detection_levels
#print detection_levels_sigma
pyplot.figure()
fig1 = pyplot.subplot(111)

fig1.set_title("Detection level vs. visual magnitude for M67")
fig1.set_xlabel("Visual Magnitude")
fig1.set_ylabel("Detection Level (Peak/median amplitude)")
pyplot.errorbar(masses_out, detection_levels, detection_levels_sigma)
pyplot.errorbar(masses_out, numpy.ones(len(masses_out))*2)
pyplot.xticks(masses_out[::3],m[::3] )
pyplot.show()
